...
function TimeseriesChart(){
    ...
    const drawChart = () => {
        ...
        var x = d3.scaleTime().range([0,width]).domain([0,d3.max(data, function(d){return d[0];})]),
            ...
            y = d3.scaleLinear().range([height,0]).domain([d3.max(data,function(d){return d[1];}),0]),
            y2 = d3.scaleLinear().range([height2,0]).domain([d3.max(data,function(d){return d[1];}),0]);;
