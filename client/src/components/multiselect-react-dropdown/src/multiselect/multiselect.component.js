...
export class Multiselect extends React.Component {
  ...
  renderSelectedList() {
    const { isObject = false, displayValue, style, singleSelect,rangeColors } = this.props;
    const { selectedValues, closeIconType } = this.state;
    return selectedValues.map((value, index,i) => (
      <span className={`chip ${ms.chip} ${singleSelect && ms.singleChip} ${this.isDisablePreSelectedValues(value) && ms.disableSelection}`} key={index} 
      style={{
      background: rangeColors[index],
      padding: "4px 10px",
      marginRight: "5px",
      marginBottom: "5px",
      borderRadius: "10px",
